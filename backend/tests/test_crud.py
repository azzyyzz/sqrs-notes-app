...
def test_get_note(db, test_user, test_note):
    ...
    another_user = models.User(
        username=f"anotheruser_{os.urandom(4).hex()}",
        hashed_password="fakehashedpass",
    )
    db.add(another_user)
    db.commit()
...
def test_get_notes(db, test_user, test_note):
    ...
    another_user = models.User(
        username=f"nouser_{os.urandom(4).hex()}",
        hashed_password="fakehashedpass",
    )
    db.add(another_user)
    db.commit()
...
def test_create_note(db, test_user):
    ...
    db_note = (
        db.query(models.Note).filter(models.Note.id == new_note.id).first()
    )
...
def test_update_note(db, test_user, test_note):
    ...
    update_data = schemas.NoteCreate(
    ...
    db_note = (
        db.query(models.Note).filter(models.Note.id == test_note.id).first()
    )
    ...
    db.add(another_user)
    db.commit()
    wrong_user_update = update_note(
        db, test_note.id, update_data, another_user.id
    )
...
def test_delete_note(db, test_user, test_note):
    ...
    another_note_data = schemas.NoteCreate(
        title="Another Note", content="Another Content"
    )
    ...
    db_note = (
        db.query(models.Note).filter(models.Note.id == another_note.id).first()
    )
    assert db_note is not None
